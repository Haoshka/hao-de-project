## Foreword
Thanks Data Engineering Zoomcamp and its community for providing this learning opportunity! 
...
- Finally by leveraging Google Looker Studio, result is presented in a dashboard so that we can easily track the SOL price action in the last 30 days whilst checking its bullish/bearish momentum.
...
### 3. Spark Data Retrieval (data per batch ingestion into DWH)
The `2.get_data_spark/` directory contains a Python script (`get_SOL_data`) that:
- Fetch Solana price via CoinGecko Rest API (https://api.coingecko.com/api/v3/coins/solana/ohlc?vs_currency=usd&days=30) as raw data - 30 days OHLC (open, high, low, close) price data with 4 hour intervals. 
...
- Partitioning the raw data (BigQuery table) per day (timestamp)
...
- **Data Lineage**:
<p align="center">
  <img width="90%" src="images/dbt-dag-hao-project.png" alt="data lineage graphic">
</p>
...
1. **Data Retrieval**:
   - The Spark script fetches Solana OHLC data from the CoinGecko API and writes it to BigQuery (DWH).
...
2. **Fetch Data**:
   - Run the .py script (stored in GCS Bucket) to fetch and load data into BigQuery directory:
     ```bash
     python 2.get_data_spark/get_SOL_data --output=<hao-de-project.hao_de_project_dataset.test_data>
     ```
...
<p align="center">
  <img width="90%" src="images/Dashboard Screenshot.JPG" alt="dashboard screenshot">
</p>
